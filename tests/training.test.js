...
describe('progressive overload', () => {
  ...
  it('verhoogt niet als er te weinig sets op het topgewicht staan', () => {
    ...
    expect(suggestNext(bench, previous).isIncrease).toBe(false);
  });
...
describe('schema-inhoud uit de kennisbasis', () => {
  ...
  it('traint elke spiergroep minstens 2× per week in elk schema', () => {
    ...
    for (const split of ['ppl6', 'upperlower5', 'upperlower4', 'fullbody3']) {
      const sessions = templates.filter((t) => t.split === split);
      for (const [group, pattern] of Object.entries(groups)) {
        const hits = sessions.filter((s) => s.exercises.some((e) => pattern.test(e.name))).length;
