...
import { addDays } from '../app/date.js';
import { collectNudges, habitState, nudgeFor } from '../app/habits.js';
...
describe('habitState — streaks', () => {
  it('telt aaneengesloten dagen', () => {
    ...
    const state = habitState(makeHabit(), logs('h1', done), new Set(), TODAY);
  ...
  it('respecteert actieve dagen bij een gewoonte die niet elke dag telt', () => {
    ...
    const state = habitState(makeHabit({ activeDays: [1, 3] }), [], new Set(), TODAY);
  ...
  it('vinkt automatisch af op basis van een andere module', () => {
    ...
    const state = habitState(makeHabit({ autoSource: 'training' }), [], auto, TODAY);
    expect(state.doneToday).toBe(true);
    expect(state.streak).toBe(2);
  });
  ...
  it('houdt de streakbalk altijd 14 dagen breed', () => {
    // Een nieuwe gewoonte mag geen één breed blok worden; de dagen ervoor
...
describe('nudgeFor — never miss twice', () => {
  it('zwijgt als er niets gemist is', () => {
    const state = habitState(makeHabit(), logs('h1', [addDays(TODAY, -1)]), new Set(), TODAY);
    expect(nudgeFor(state, TODAY)).toBeNull();
  });

  it('is mild na één gemiste dag', () => {
    const state = habitState(makeHabit(), logs('h1', [addDays(TODAY, -2)]), new Set(), TODAY);
