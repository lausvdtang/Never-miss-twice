...
export const SPLIT_LABELS: Record<WorkoutTemplate['split'], string> = {
 ...
 */
export function splitForDays(days: number): WorkoutTemplate['split'] {
 ...
 * Lesdagen (ma/wo/vr) hebben een vroege start; op die dagen plannen we bij
 * voorkeur de sessies die de gebruiker als eerste wil afvinken.
 */
export function defaultDayAssignment(days: number): number[] {
  switch (days) {
    case 6:
      return [1, 2, 3, 4, 5, 6]; // ma t/m za, zo = rust of actief herstel (§8.2)
    case 5:
      return [1, 2, 4, 5, 6];
    case 4:
      return [1, 2, 4, 6];
 ...
 */
export const DEFAULT_FOOD_PRESETS: FoodPreset[] = [
...
export const PREP_FORMULA: {
  component: 'eiwit' | 'koolhydraat' | 'groente' | 'smaakmaker';
  label: string;
  suggestions: string[];
}[] = [
  {
...
export function buildDefaultHabits(now: string): Habit[] {
  const base = { archived: false, createdAt: now, activeDays: [] as number[] };
  return [
    {
      ...base,
      id: 'hab_training',
...
export const INTENTION_TEMPLATES: { trigger: string; action: string }[] = [
...
export const CARDIO_GUIDANCE = {
  ...
  runMinutesMin: 20,
  runMinutesMax: 30,
  /** Interference effect: niet vlak vóór krachttraining. */
  interferenceWarning:
...
export const TREND_THRESHOLDS = {
  /** >1% lichaamsgewicht/week verlies → suggereer meer eten. */
  fastLossPctPerWeek: 1,
  /** Geen verandering in 3-4 weken bij lean bulk → +100-150 kcal. */
  stallWeeks: 3,
  stallBumpKcal: [100, 150] as const,
};
